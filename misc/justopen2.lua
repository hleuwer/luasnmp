...
printf("password successfully changed for user %s.", sessold.user)

sleep(T)

local sessnew
if clonenew then
  sessnew, err = sessold:clone{password = newpw}
else
...
if cloneold then
  printf("\n===== Clone OLD session")
  sessold2, err = sessnew:clone{password=oldpw}
else
  if reopenold then
    printf("\n===== Reopen OLD session")
